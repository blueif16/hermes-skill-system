...
10. **Shared/upstream changes are local-first, then promoted.** Build + VERIFY a shared-component fix in the product locally first; promote it upstream only after a real run proves it, made robust for ANY environment — never author an unverified change upstream, never patch/hack. *(`references/debug-tuning-loop.md`.)*
